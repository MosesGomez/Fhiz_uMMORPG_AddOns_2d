...
{
    [Header("[-=-=-=- UCE QUESTS -=-=-=-]")] [Tooltip("[Required] Contains active and completed quests (=all)")]
    ...
    public void UCE_checkQuestCompletion(int index)
    {
        ...
        {
            UCE_ShowPopup(questCompletePopup.message + UCE_quests[index].name, questCompletePopup.iconId,
                questCompletePopup.soundId);
    ...
    public int[] checkGatheredItems(UCE_Quest quest)
    {
        ...
        {
            j = i;
            gathered[j] = Mathf.Min(InventoryCount(new Item(quest.gatherTarget[j].target)),
                quest.gatherTarget[j].amount);
        }
